...
레지스트리에 게시하지 않고 **git 소스에서 직접** `npx`로 설치합니다. 클론/빌드 불필요 —
`prepare` 스크립트가 설치 시 자동 빌드합니다.

```bash
# 1) 설치 마법사 — provider 선택 → 인증 → 연결 테스트 → 설정 저장
npx -y github:harry-jang/mcp-wizard-sample setup

# 2) Claude Code에 등록 — 스코프는 사용자가 선택
#    팀 공유 없이 나만 쓰려면 (모든 프로젝트에서):
claude mcp add -s user wizard-connector -- npx -y github:harry-jang/mcp-wizard-sample
#    특정 프로젝트에서 팀과 공유하려면 (그 프로젝트의 .mcp.json에 기록):
claude mcp add -s project wizard-connector -- npx -y github:harry-jang/mcp-wizard-sample
```

`npx -y github:harry-jang/mcp-wizard-sample` (인자 없음)는 **MCP 서버**로 동작하고, Claude Code가 이 명령으로
서버를 띄웁니다. `setup` 인자를 붙이면 **마법사**가 실행됩니다 (단일 bin이 분기). 버전 고정이 필요하면
`github:harry-jang/mcp-wizard-sample#v0.1.0` 처럼 태그/커밋을 붙이세요.

> **참고:** 이 repo는 MCP 서버 **소스**이지 소비 프로젝트가 아니므로 `.mcp.json`을 커밋하지 않습니다
> (`.gitignore` 처리). MCP 등록 위치(project/user 스코프)는 위처럼 **소비자가 직접 선택**합니다.
